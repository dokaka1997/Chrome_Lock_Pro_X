(function initClpxI18n(globalScope) {
  ...
  const messages = {
    en: {
      ...
      'popup.hero.setup_text': 'Open Settings to create the first password.',
    ...
    vi: {
      ...
      'popup.hero.setup_text': 'Mở trang Cài đặt để tạo mật khẩu lần đầu.',
  ...
  Object.assign(messages.en, {
    ...
    'options.policy.clear_history_on_lockout_toggle': 'Clear Chrome browsing history when lockout is triggered by too many failed attempts',
    'options.policy.clear_history_on_lockout_hint': 'When enabled, the extension will request one-time permission to clear browsing history after the failed-attempt limit is reached.',
    ...
    'options.messages.history_permission_denied': 'The browsing history permission was not granted, so this setting was not saved.',
    'popup.audio.title': 'Current tab volume',
    'popup.audio.mute': 'Mute',
    'popup.audio.reset': 'Reset 100%',
    'popup.audio.inactive': 'Set a value other than 100% to enable per-tab volume control.',
    'popup.audio.active': 'Per-tab volume is active at {{volume}}%.',
    'popup.audio.boosting': 'Boost is active at {{volume}}%. Higher levels can distort the sound.',
    'popup.audio.muted': 'This tab is muted through the extension audio pipeline.',
    'popup.audio.unsupported': 'Volume control only works on normal http/https tabs.',
    'popup.audio.error.state': 'Could not read the current tab volume state.',
    'popup.audio.error.control_failed': 'Could not change the current tab volume.',
    ...
    'background.error.audio_tab_missing': 'Could not find the tab for audio control.',
    'background.error.audio_capture_failed': 'Chrome could not start capturing audio for this tab.',
    'background.error.audio_control_failed': 'Could not change the audio level for this tab.',
  ...
  Object.assign(messages.vi, {
    ...
    'options.policy.clear_history_on_lockout_toggle': 'Xóa lịch sử duyệt web của Chrome khi bị khóa tạm thời do nhập sai quá nhiều lần',
    'options.policy.clear_history_on_lockout_hint': 'Khi bật, extension sẽ xin quyền một lần để xóa lịch sử duyệt web sau khi chạm ngưỡng nhập sai.',
    ...
    'options.messages.history_permission_denied': 'Bạn chưa cấp quyền lịch sử duyệt web, nên cài đặt này chưa được lưu.',
    'popup.audio.title': 'Âm lượng tab hiện tại',
    'popup.audio.mute': 'Tắt tiếng',
    'popup.audio.reset': 'Về 100%',
    'popup.audio.inactive': 'Hãy đặt giá trị khác 100% để bật điều khiển âm lượng riêng cho tab này.',
    'popup.audio.active': 'Âm lượng riêng của tab đang ở mức {{volume}}%.',
    'popup.audio.boosting': 'Đang tăng âm lên {{volume}}%. Mức cao có thể làm méo tiếng.',
    'popup.audio.muted': 'Tab này đang bị tắt tiếng qua audio pipeline của extension.',
    'popup.audio.unsupported': 'Điều khiển âm lượng chỉ hoạt động trên tab http/https thông thường.',
    'popup.audio.error.state': 'Không đọc được trạng thái âm lượng của tab hiện tại.',
    'popup.audio.error.control_failed': 'Không thể thay đổi âm lượng của tab hiện tại.',
    ...
    'background.error.audio_tab_missing': 'Không tìm thấy tab để điều khiển âm thanh.',
    'background.error.audio_capture_failed': 'Chrome không thể bắt đầu capture âm thanh cho tab này.',
    'background.error.audio_control_failed': 'Không thể thay đổi mức âm lượng của tab này.',
  ...
  function looksLikeMojibake(value) {
    return /(?:Ã|Ä|Æ|â|á»|áº|á¼|á½|á¾|á¿)/.test(String(value || ''));
  }
  ...
  function decodeMojibakeOnce(value) {
    ...
    for (const char of text) {
      ...
      const mappedByte = WINDOWS_1252_TO_BYTE.get(codePoint);
  ...
  function translate(language, key, values = {}) {
    ...
    const template = repairMojibake(dictionary[key] ?? fallbackDictionary[key] ?? key);
